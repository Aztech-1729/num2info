...
async def resolve_username_full(username: str) -> dict | None:
    ...
    Works even if user never interacted with the bot or isn't in any shared group.
    """
    global tg_client
    if not tg_client:
...
async def fetch_profile_photo(identifier) -> bytes | None:
    """
    Download the first profile photo of a user as bytes.
    identifier = username string OR numeric user_id int.
    Returns raw bytes or None if no photo available.
    """
    global tg_client
    if not tg_client:
        return None
    try:
        import io
        buf = io.BytesIO()
        result = await tg_client.download_profile_photo(identifier, file=buf)
        if result:
            buf.seek(0)
            return buf.read()
        return None
    except Exception as e:
        logger.warning(f"Could not download profile photo for {identifier}: {e}")
        return None
...
class PhoneIntelligenceBot:
    ...
    async def fetch_username_phone_data(self, user_id: str):
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(self.username_api_endpoint, params=params, timeout=15) as response:
                    if response.status == 200:
                        data = await response.json()
                        return data
                    return None
    ...
    async def handle_phone_number(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Check for username or user ID first
        input_type, input_value = self.extract_username_or_userid(raw_input)
    ...
    async def _send_lookup_response(self, bot, chat_id: int, text: str, tg_info: dict | None = None, reply_to: int | None = None):
        ...
        photo_bytes = None
        if tg_info:
            identifier = tg_info.get("username") or tg_info.get("id")
            if identifier and identifier != "N/A":
                photo_bytes = await fetch_profile_photo(identifier)
